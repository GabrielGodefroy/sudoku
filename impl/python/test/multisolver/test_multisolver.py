...
def test_multisolver_from_empty_grid():
    ...
    size = 9
    nb_grids = 5
    initial_empty_grids = [[0 for i in range(size)] for j in range(size)]
    # when
    solutions = [*islice(multisolve(initial_empty_grids), nb_grids)]
    # then
    for solution in solutions:
        assert type(solutions) is list
        assert check_solution(np.array(solution))

    for i in range(len(solutions)):
        for j in range(i):
            assert (
                np.array(solutions[i]) != np.array(solutions[j])
